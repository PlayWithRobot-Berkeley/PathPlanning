...
class Controller(object):
    ...
    def __init__(self, Kp, Ki, Kd, Kw, limb,
            controller_type: ControllerType = ControllerType.FEED_FORWARD):
        """
        Constructor:
    ...
    def step_control(self, t):
        ...
        u = u_ff 
        if self._type == ControllerType.PD or self._type == ControllerType.PID:
            u += self._Kp * error
            u += self._Kd * curr_derivative
            if self._type == ControllerType.PID:
                u += self._Ki * self._IntError
