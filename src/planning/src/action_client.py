...
def main():
    ...
    while joint_idx < 4:
        try:
            start = float(input(f"Start for {joint_idx}: "))
            end = float(input(f"End for {joint_idx}: "))
            goal.request.start_state.joint_state.position[joint_idx] = start
            target_joint_angles[joint_idx] = end
            joint_idx += 1
        except:
            pass
